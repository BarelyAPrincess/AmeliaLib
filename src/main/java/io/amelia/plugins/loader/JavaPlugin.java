...
public abstract class JavaPlugin extends Plugin
{
	 ...
	 */
	public static <T extends Plugin> T getPlugin( @Nonnull Class<T> clazz )
	{
	 ...
	 * @throws IllegalStateException    if called from the static initializer for given Plugin
	 */
	public static Plugin getProvidingPlugin( @Nonnull Class<?> pluginClass )
	{
		final ClassLoader classLoader = pluginClass.getClassLoader();
		if ( !( classLoader instanceof PluginClassLoader ) )
			throw new IllegalArgumentException( pluginClass + " is not provided by " + PluginClassLoader.class );
		Plugin plugin = ( ( PluginClassLoader ) classLoader ).plugin;
		if ( plugin == null )
			throw new IllegalStateException( "Cannot get plugin for " + pluginClass + " from a static initializer" );
		return plugin;
	}
	...
	@Override
	public final void init( PluginLoader loader, PluginMeta meta, Path dataPath, Path pluginPath, ClassLoader classLoader )
	{
		this.loader = loader;
		...
		this.configPath = Paths.get( "config.yaml" ).resolve( dataPath );
	}
	...
	@Override
	public void reloadConfig() throws ParcelableException.Error, IOException
	{
