 ...
 *     permission: io.amelia.destruction
...
public class PluginMeta extends VendorMeta
{
	...
	public List<MavenReference> getLibraries()
	{
		return getList( "libraries", "|", e -> {
			...
			{
				DefaultPlugins.L.severe( "Could not parse the library '" + e + "' for plugin '" + getName() + "', expected pattern 'group:name:version'. Unless fixed, it will be ignored.", ee );
			}
			return null;
		} );
	}
	...
	/**
	 * Gives a list of other plugins that the plugin requires for full functionality. The {@link DefaultPlugins} will make
