 ...
 * @see Path
 */
public interface ImplPath<Self extends ImplPath> extends Path
{
	Self create( String... nodes );

	default Self create( Path path )
	{
		return create( Strs.split( path.toString(), File.pathSeparator ).toArray( String[]::new ) );
	}
	...
	{
		if ( other == null || other.getNameCount() > getNameCount() )
			return false;
		for ( int i = getNameCount() - other.getNameCount(); i < other.getNameCount(); i++ )
	...
	{
		return endsWith( Paths.get( other ) );
	}
	...
	{
		return getNameCount() < 1 ? null : getName( getNameCount() - 1 );
	}
	 ...
	 *                                  elements
	 */
	default Self getName( int index )
	{
		if ( index < 0 || index > getNameCount() || getNameCount() < 1 )
	...
	{
		return getNodes()[getNameCount() - 1];
	}
	...
	{
		return Arrays.stream( getNames() ).map( Paths::get ).collect( Collectors.toList() ).iterator();
	}
	 ...
	 *                                  against this path
	 */
	default Self relativize( Path other )
	{
		if ( getFileSystem() == other.getFileSystem() )
		{
			Self self = toAbsolutePath();
			other = other.toAbsolutePath();

			Namespace ns = Namespace.of( self.getNames() );
			for ( int i = 0; i < self.getNameCount(); i++ )
			{
				if ( Namespace.of( IO.getNames( other ) ).startsWith( ns ) )
					return create( Strs.repeat( "../", ns.getNodeCount() ) + other.subpath( ns.getNodeCount(), other.getNameCount() ) );
				ns = ns.pop();
			}
			return create( other );
		}
	 ...
	 * @see #relativize
	 */
	default Self resolve( Path other )
	{
		if ( other.isAbsolute() )
			return create( other );
		Namespace ns = Namespace.of( getNames() );
		while ( other.startsWith( ".." ) )
		{
			other = other.subpath( 1, other.getNameCount() );
			ns = ns.pop();
		}
		ns = ns.append( IO.getNames( other ) );
		return create( ns.getNames() );
	}
	 ...
	 */
	default Self resolve( String other )
	{
	 ...
	 * @see #resolve(Path)
	 */
	default Self resolveSibling( Path other )
	{
		return ( Self ) getParent().resolve( other );
	}
	 ...
	 * @see FileSystem#getPath
	 */
	default Self resolveSibling( String other )
	{
		return ( Self ) getParent().resolve( other );
	}
	...
	{
		return Namespace.of( getNames() ).startsWith( Namespace.of( IO.getNames( other ) ) );
	}
	 ...
	 */
	default Self subpath( int beginIndex, int endIndex )
	{
		if ( beginIndex < 0 || beginIndex >= getNameCount() || endIndex < beginIndex || endIndex > getNameCount() )
			throw new IllegalArgumentException();
		return create( Arrays.copyOfRange( getNames(), beginIndex, endIndex ) );
	}
	 ...
	 */
	default Self toAbsolutePath()
	{
		Self self = getDefaultPath();
		if ( !self.isAbsolute() )
			throw new IllegalArgumentException( "Default path is not absolute." );
		return ( Self ) self.resolve( this );
	}
	 ...
	 *                           system property {@code user.dir}
	 */
	default Self toRealPath( LinkOption... options ) throws IOException
	{
		throw new UnsupportedOperationException();
	}
	...
	default URI toUri()
	{
		...
		if ( fileSystem == null )
			return null;
		return URI.create( fileSystem.provider().getScheme() + "://" + toString() );
	}
}
