 ...
 * Reasons for a transfer of power would include low-latency and member going silent, no "I Am Healthy" packets.
 * <p>
 * Each cluster member consistency records the latency seen by the cluster members. If the MASTER has gone
 ...
 * <p>
 * When new members wish to join, it will broadcast a cluster status packet, asking for each MEMBER of the cluster
 ...
 * is only a minor version. Major versions will always shutdown and request they are manually updated with the --update
 ...
 * Depending on if the update was expected, the old MEMBERS will try to coordinate with the new MEMBERS for a peaceful
 * takeover.
 */
public enum ClusterRole
{
	...
	/**
	 * Negotiating with the cluster, waiting to be promoted by the cluster.
	 */
