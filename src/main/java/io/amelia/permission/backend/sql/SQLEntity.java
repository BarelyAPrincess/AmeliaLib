...
public class SQLEntity extends PermissibleEntity
{
	...
	public void remove()
	{
		...
		try
		{
			// db.queryUpdate( String.format( "DELETE FROM `permissions_entity` WHERE `owner` = '%s' AND `type` = '0';", getUuid() ) );
			// db.queryUpdate( String.format( "DELETE FROM `permissions_groups` WHERE `parent` = '%s' AND `type` = '0';", getUuid() ) );
	...
	public void save()
	{
		...
		try
		{
			...
			for ( ChildPermission child : children )
			{
				...
				// db.queryUpdate( String.format( "INSERT INTO `permissions_entity` (`owner`,`type`,`refs`,`permission`,`value`) VALUES ('%s','0','%s','%s','%s');", getUuid(), child.getReferences().join(), perm.getNamespace(),
				// child.getObject() ) );
			...
			// db.queryUpdate( String.format( "INSERT INTO `permissions_groups` (`child`, `parent`, `type`, `refs`) VALUES ('%s', '%s', '0', '%s');", entry.getKey().getUuid(), getUuid(), entry.getValue().join() ) );
		}
		catch ( DatabaseException e )
		{
			throw new RuntimeException( e );
