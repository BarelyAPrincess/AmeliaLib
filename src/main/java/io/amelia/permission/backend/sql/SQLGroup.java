...
public class SQLGroup extends PermissibleGroup
{
	...
	public void reloadGroups()
	{
		...
		try
		{
			...
			// ResultSet rs = db.query( "SELECT * FROM `permissions_groups` WHERE `parent` = '" + getUuid() + "' AND `type` = '1';" );
	...
	public void reloadPermissions()
	{
		...
		try
		{
			...
			// ResultSet rs = db.query( "SELECT * FROM `permissions_entity` WHERE `owner` = ? AND `type` = '1';", getUuid() );
	...
	public void remove()
	{
		...
		try
		{
			// db.queryUpdate( String.format( "DELETE FROM `permissions_entity` WHERE `owner` = '%s' AND `type` = '1';", getUuid() ) );
			// db.queryUpdate( String.format( "DELETE FROM `permissions_groups` WHERE `parent` = '%s' AND `type` = '1';", getUuid() ) );
	...
	public void save()
	{
		...
		try
		{
			...
			for ( ChildPermission child : children )
			{
				...
				// db.queryUpdate( String.format( "INSERT INTO `permissions_entity` (`owner`,`type`,`refs`,`permission`,`value`) VALUES ('%s','1','%s','%s','%s');", getUuid(), child.getReferences().join(), perm.getNamespace(),
				// child.getObject() ) );
			...
			// db.queryUpdate( String.format( "INSERT INTO `permissions_groups` (`child`, `parent`, `type`, `refs`) VALUES ('%s', '%s', '1', '%s');", entry.getKey().getUuid(), getUuid(), entry.getValue().join() ) );
		}
		catch ( DatabaseException e )
		{
			throw new RuntimeException( e );
