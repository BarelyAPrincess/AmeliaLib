...
public class PermissionGuard implements PermissionService
{
	...
	{
		return checkPermission( entity.uuid(), perm, "" );
	}
	...
	{
		return entities.values().stream().filter( e -> e.getUuid().toLowerCase().startsWith( query.toLowerCase() ) ).collect( Collectors.toSet() );
	}
	...
	{
		return groups.values().stream().filter( g -> g.getUuid().toLowerCase().startsWith( query.toLowerCase() ) ).collect( Collectors.toList() );
	}
	...
	public static void setDefaultGroup( PermissibleGroup group, References refs )
	{
		...
		backend.setDefaultGroup( group.getUuid(), refs );
