...
public abstract class PermissibleEntity implements UserPrincipal
{
	...
	public PermissibleEntity( String uuid )
	{
		if ( PermissionGuard.isDebug() )
			PermissionGuard.L.info( String.format( "%sThe %s `%s` has been created.", EnumColor.YELLOW, isGroup() ? "group" : "entity", uuid ) );
	...
	protected void addGroup0( PermissibleGroup group, References refs )
	{
		...
		if ( isDebug() )
			PermissionGuard.L.info( String.format( "%sThe group `%s` with reference `%s` was attached to entity `%s`.", EnumColor.YELLOW, group.getUuid(), refs.join(), getUuid() ) );
	}
	...
	protected final void addPermission( ChildPermission perm, References refs )
	{
		...
		{
			if ( isDebug() )
				PermissionGuard.L.info( String.format( "%sThe permission `%s` with reference `%s` is already attached to entity `%s`.", EnumColor.YELLOW, perm.getPermission().getNamespace(), refs == null ? "null" : refs.join(), getUuid() ) );
			return;
		}
		...
		if ( isDebug() )
			PermissionGuard.L.info( String.format( "%sThe permission `%s` with reference `%s` was attached to entity `%s`.", EnumColor.YELLOW, perm.getPermission().getNamespace(), refs.join(), getUuid() ) );
	...
	protected final void addTimedPermission( ChildPermission perm, TimedReferences refs )
	{
		permissions.put( perm, refs );
		if ( isDebug() )
			PermissionGuard.L.info( String.format( "%sThe permission `%s` with reference `%s` was attached to entity `%s`.", EnumColor.YELLOW, perm.getPermission().getNamespace(), refs.toString(), getUuid() ) );
		recalculatePermissions();
	}
	...
	public PermissionResult checkPermission( Permission perm, References refs )
	{
		...
		if ( isDebug() && !perm.getNamespace().equals( PermissionDefault.OP.getNamespace() ) )
			PermissionGuard.L.info( EnumColor.YELLOW + "Entity `" + getUuid() + "` checked for permission `" + perm.getNamespace() + "`" + ( refs.isEmpty() ? "" : " with reference `" + refs.toString() + "`" ) + " with result `" + result + "`" );
	...
	protected ChildPermission getChildPermission( Permission perm, References refs )
	{
		...
		// Loader.L.debug( "Get PermissionChild on " + ( isGroup() ? "group" : "entity" ) + " " + getUuid() + " with result " + ( result != null ) );
	}
	...
	/**
	 * Return uuid of permission entity (Entity or Group) User should be equal to User's uuid on the server
	 *
	 * @return uuid
	 */
	...
	{
		return "none".equalsIgnoreCase( getUuid() );
	}
	...
	{
		return "root".equalsIgnoreCase( getUuid() );
	}
	...
	public void reload()
	{
		...
		if ( isDebug() )
			PermissionGuard.L.info( EnumColor.YELLOW + "Entity '" + getUuid() + "' being reloaded from backend" );
