/**
 * This software may be modified and distributed under the terms
 * of the MIT license.  See the LICENSE file for details.
 * <p>
 * Copyright (c) 2019 Amelia Sara Greene <barelyaprincess@gmail.com>
 * Copyright (c) 2019 Penoaks Publishing LLC <development@penoaks.com>
 * <p>
 * All Rights Reserved.
 */
package io.amelia.scripting.groovy;

import java.io.PrintStream;
import java.io.UnsupportedEncodingException;

import groovy.lang.Binding;
import groovy.lang.Script;
import io.amelia.scripting.ScriptingFactory;
import io.amelia.scripting.HttpScriptingRequest;
import io.netty.buffer.ByteBufOutputStream;

/**
 * Implements the builtin API by being extent by groovy nested classes
 */
public class NestedScript extends Script
{
	HttpScriptingRequest httpScriptingRequest;

	@SuppressWarnings( "unchecked" )
	public NestedScript()
	{
		httpScriptingRequest = HttpScriptingRequest.getThreadReference();
		Binding binding = new Binding( httpScriptingRequest.getBinding().getVariables() );
		try
		{
			binding.setProperty( "out", new PrintStream( new ByteBufOutputStream( httpScriptingRequest.getScriptingFactory().getOutputStream() ), true, httpScriptingRequest.getScriptingFactory().getCharset().name() ) );
		}
		catch ( UnsupportedEncodingException e )
		{
			e.printStackTrace();
		}
		// setBinding( binding );
		getBinding().getVariables().putAll( binding.getVariables() );
		// Setting the binding directly was causing an override of variable value from last class instance.
		// Possibly a link to the groovy MetaClass, hmm, thoughts on how to make use of this...
	}

	public ScriptingFactory getScriptingFactory()
	{
		return httpScriptingRequest.getScriptingFactory();
	}

	@Override
	public Object run()
	{
		return null;
	}
}
