...
public class GroovyServerPagesEngine implements ScriptingEngine
{
	...
	static
	{
		...
		if ( ConfigRegistry.config.getValue( PHP_STYLE_TAGS ) )
		{
			MARKERS.add( new Triplet<>( "<?", null, "?>" ) );
			MARKERS.add( new Triplet<>( "<?=", "echo", "?>" ) );
		}
			...
			if ( script == null )
			{
				...
				while ( fullFileIndex < source.length() )
				{
					...
					if ( startIndex > -1 )
					{
						...
						if ( endIndex == -1 )
							throw new ScriptingException.Error( ReportingLevel.E_PARSE, String.format( "Found starting marker '%s' at line %s, expected close marker '%s' not found.", activeMarker.getStart(), Strs.countMatches( output.toString(), '\n' ) + 1, activeMarker.getEnd() ) );
				...
				context.setBaseSource( output.toString() );
